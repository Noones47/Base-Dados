Public Class FzrPedido
    ...
    Public Sub LoadGrid2()
        ...
        SQL.Executar("SELECT Pedido.id_pedido,Empregado.nome,Empregado.id_emp,Local.id_local,Produto.designacao,Prod_Pedido.quantidade_paga,Pedido.total_preco,Produto.preco_venda_unidade,Produto.preco_custo, Pedido.dia,Pedido.hora, Local.localizacao, Local.tempo_repor FROM Local,Pedido,Produto,Prod_Pedido,Empregado WHERE Prod_Pedido.id_pedido = Pedido.id_pedido and Pedido.id_emp = Empregado.id_emp and Prod_Pedido.id_produto = Produto.id_produto and Pedido.id_local = Local.id_local and Local.localizacao LIKE '%" & cdxlocal.Text & "'")
